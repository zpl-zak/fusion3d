	...
	IndexedModel map;
	{
		for (int i = 0; i < level.GetWidth(); i++)
		{
			for (int j = 0; j < level.GetHeight(); j++)
			{
				...
				map.AddVertex(Vector3f(i * SPOT_W, Y1, j * SPOT_L));
				map.AddTexCoord(Vector2f(XL, YL));
				map.AddVertex(Vector3f((i + 1) * SPOT_W, Y2, j * SPOT_L));
				map.AddTexCoord(Vector2f(XH, YL));
				map.AddVertex(Vector3f((i + 1) * SPOT_W, Y3, (j + 1) * SPOT_L));
				map.AddTexCoord(Vector2f(XH, YH));
				map.AddVertex(Vector3f(i * SPOT_W, Y4, (j + 1) * SPOT_L));
